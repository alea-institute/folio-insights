"""Aporetic resolution path (07-05a Task 1).

A ContestResolutionEvent with ``resolution_path="aporetic"`` records that
the dispute is unresolvable on the present record; the shard remains
contested-but-acknowledged. Aporetic is a 1st-class GOV-05 outcome — distinct
from arbiter and distinguo, NOT a majority-vote substitute.
"""
...
async def test_aporetic_resolution_appends_and_audits() -> None:
    """Aporetic resolution is preserved verbatim in the log audit."""
    ...
    contest = ContestEvent(
        ...
        shard_iri=SHARD,
        voter_did=BOB,
        position_text="Unresolvable from present record.",
    )
    ...
    resolution = ContestResolutionEvent(
        ...
        shard_iri=SHARD,
        resolution_path="aporetic",
    )
    await validate_contest_resolution(resolution, log=log)
    persisted = await log.append(resolution)
    assert persisted.resolution_path == "aporetic"
    # The aporetic resolution is visible in the audit trail.
    collected = [ev async for ev in log.iter_events(CORPUS)]
    assert collected[-1].resolution_path == "aporetic"
