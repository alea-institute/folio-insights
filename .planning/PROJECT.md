...
---
*Last updated: 2026-05-27 — Phase 05 (Content Versioning §6.4) complete; SHARD-09 validated. New `revision/` package (dep-leak guard forced the boundary — RDF/SHACL out of `shards/`): async store-backed `edit_shard_content()` (full PRD §6.4 signature, D-01/D-03) over an in-memory `ShardStore` seam (D-02, Phase-13 swaps Oxigraph in), enriched `ContentEdit` (dotted `field_path` + `rationale` + `AttestedSignature` slot, D-04/D-05), central 10-member `IMMUTABLE_FIELD_PATHS` gate (D-06), and `get_shard_at(iri, t)` reverse-replay on a deep copy (D-09, exit 3, 1000-example Hypothesis suite). Two-layer forward-only enforcement (D-07): authoritative Pydantic `@model_validator` + defense-in-depth pyshacl `sh:sparql` shape — the literal "SHACL guard" (exit 2) ships now, live-spiked against pyshacl 0.31.0 (back-dated insert → conforms=False). Real deterministic `canonical_content_hash` now (D-05; Phase 6 swaps JCS). Code review caught 2 blockers + 3 warnings — in-place store corruption (CR-01), `transaction_time` leaking into the content hash (CR-02), `add_edit`/`get_field` path-injection + missing rollback (WR-01/02/03) — ALL fixed with 25 directed regression tests. 218 shards+revision tests green. Stubbed for later: ed25519/JCS signing → Phase 6, persistent Oxigraph → Phase 13, full SHACL-Hybrid suite → Phase 11. v2.0 milestone now 6/24 phases (00–05) done.*
