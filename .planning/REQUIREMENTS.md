...
| ID | Priority | Requirement | Source | Acceptance |
|----|----------|-------------|--------|------------|
| SEC-01 | P1 | **Phase 0 HARD GATE** — RDF-12 annotation pattern validates all PRD §20 query patterns, OR pivot to Apache Jena Fuseki (pre-specified fallback) | STACK RISK-3; Scope session 2026-04-22 | Phase 0 spike exits with decision artifact: `keep=pyoxigraph` OR `pivot=fuseki`; both branches have ROADMAP entries | **SATISFIED 2026-04-23** — Phase 0 Plan 00-08 `.planning/phases/00-foundations-hard-gate/00-DECISION.md` verdict=keep=pyoxigraph |
| SEC-02 | P1 | Phase 0 exit gates (5): P95 SPARQL < 500ms @ 1M triples; worker image < 500MB; SSR latency < 200ms; RDF-12 annotation pattern works; Dagger CI reproducible | Research §Phase 0 | All 5 gates measured in Phase 0; failures trigger scoped pivots (per SEC-01 branch) | **SATISFIED 2026-04-23** — Gates 1/2/3/5 PASS (13/13 gold; 116.95ms P95; 74.6MB; bit-identical digest). Gate 4 deferred (harness-ready, NOT pivot trigger per OQ4). See `00-08-SUMMARY.md` |
